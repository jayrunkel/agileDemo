...
//	  res = await CLIENT.query('SELECT DISTINCT(subject) FROM ticket WHERE subject ILIKE $1 LIMIT $2', values)
		res = await CLIENT.query('SELECT DISTINCT ("ticketDetails"->>\'subject\') as completion FROM "ticketJSONB" WHERE "ticketDetails"->>\'subject\' ILIKE $1 LIMIT $2', values)
	} catch (err) {
		console.log(err.stack)
	}

	return res ? res.rows.map(c => c.completion) : null
}
...
export async function search(phrase, fuzzyDistance, numResults) {
	...
	res = await CLIENT.query('SELECT "ticketNumber", "ticketDetails" \
                            FROM (SELECT "ticketNumber", "ticketDetails"->>\'subject\' as subject, "ticketDetails", unnest(string_to_array(jsonb_array_elements("ticketDetails"->\'comments\')->>\'comment\', \' \'::text) || string_to_array("ticketDetails"->>\'subject\', \' \'::text)) \
                                  FROM "ticketJSONB") as u \
